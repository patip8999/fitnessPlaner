...
import {
  ...
  ɵɵdefineNgModule,
  ɵɵinject
} from "./chunk-J2AK2P3M.js";
import {
  require_cjs
} from "./chunk-TASRQ32Q.js";
import {
  __async,
  __toESM
...
var appCheckInstance$ = (0, import_rxjs2.timer)(0, 300).pipe((0, import_operators2.concatMap)(() => (0, import_rxjs2.from)(ɵgetAllInstancesOf(ɵAPP_CHECK_PROVIDER_NAME))), (0, import_operators2.distinct)());
...
var AngularFireAuth = class _AngularFireAuth {
  ...
  constructor(options, name2, platformId, zone, schedulers, useEmulator, settings, tenantId, languageCode, useDeviceLanguage, persistence, _appCheckInstances) {
    const logins = new import_rxjs3.Subject();
    const auth = (0, import_rxjs3.of)(void 0).pipe((0, import_operators3.observeOn)(schedulers.outsideAngular), (0, import_operators3.switchMap)(() => zone.runOutsideAngular(() => import("./dist-H3JDKDSL.js"))), (0, import_operators3.map)(() => ɵfirebaseAppFactory(options, zone, name2)), (0, import_operators3.map)((app) => ɵauthFactory(app, zone, useEmulator, tenantId, languageCode, useDeviceLanguage, settings, persistence)), (0, import_operators3.shareReplay)({
      bufferSize: 1,
      refCount: false
    }));
    if (isPlatformServer(platformId)) {
      this.authState = this.user = this.idToken = this.idTokenResult = this.credential = (0, import_rxjs3.of)(null);
    } else {
      auth.pipe((0, import_operators3.first)()).subscribe();
      const redirectResult = auth.pipe((0, import_operators3.switchMap)((auth2) => auth2.getRedirectResult().then((it) => it, () => null)), keepUnstableUntilFirst, (0, import_operators3.shareReplay)({
        bufferSize: 1,
        refCount: false
      }));
      const authStateChanged = auth.pipe((0, import_operators3.switchMap)((auth2) => new import_rxjs3.Observable((sub) => ({
        unsubscribe: zone.runOutsideAngular(() => auth2.onAuthStateChanged((next) => sub.next(next), (err) => sub.error(err), () => sub.complete()))
      }))));
      const idTokenChanged = auth.pipe((0, import_operators3.switchMap)((auth2) => new import_rxjs3.Observable((sub) => ({
        unsubscribe: zone.runOutsideAngular(() => auth2.onIdTokenChanged((next) => sub.next(next), (err) => sub.error(err), () => sub.complete()))
      }))));
      this.authState = redirectResult.pipe((0, import_operators3.switchMapTo)(authStateChanged), (0, import_operators3.subscribeOn)(schedulers.outsideAngular), (0, import_operators3.observeOn)(schedulers.insideAngular));
      this.user = redirectResult.pipe((0, import_operators3.switchMapTo)(idTokenChanged), (0, import_operators3.subscribeOn)(schedulers.outsideAngular), (0, import_operators3.observeOn)(schedulers.insideAngular));
      this.idToken = this.user.pipe((0, import_operators3.switchMap)((user) => user ? (0, import_rxjs3.from)(user.getIdToken()) : (0, import_rxjs3.of)(null)));
      this.idTokenResult = this.user.pipe((0, import_operators3.switchMap)((user) => user ? (0, import_rxjs3.from)(user.getIdTokenResult()) : (0, import_rxjs3.of)(null)));
      this.credential = (0, import_rxjs3.merge)(
        redirectResult,
        logins,
        // pipe in null authState to make credential zipable, just a weird devexp if
        // authState and user go null to still have a credential
        this.authState.pipe((0, import_operators3.filter)((it) => !it))
      ).pipe(
        // handle the { user: { } } when a user is already logged in, rather have null
        // TODO handle the type corcersion better
        (0, import_operators3.map)((credential) => credential?.user ? credential : null),
        (0, import_operators3.subscribeOn)(schedulers.outsideAngular),
        (0, import_operators3.observeOn)(schedulers.insideAngular)
      );
    }
...
@firebase/app-check/dist/esm/index.esm2017.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-S6SHCHLU.js.map
