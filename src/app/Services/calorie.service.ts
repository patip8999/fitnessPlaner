...
export class CalorieService {
 client = inject(AngularFirestore);
    ...
    saveTdee(calorieData: { tdee: number; gender: string; age: number; height: number; weight: number; activityLevel: number }): Promise<void> {
    ...
    // Metoda do ładowania ostatniego zapisanego TDEE z Firebase
            ...
            this.client
              ...
              .subscribe((querySnapshot) => {
                if (!querySnapshot.empty) {
                  ...
                  observer.next(doc); // Zwracamy dane z ostatniego zapisanego TDEE
                } else {
                  observer.next(null); // Brak danych
                }
              });
          } else {
