...
export class ThemeService {
  ...
  setGradient(color: string) {
    ...
    this.saveUserTheme(color);
  }
  private saveUserTheme(color: string) {
    const user = this.afAuth.currentUser;
    user.then((currentUser) => {
      if (currentUser) {
        this.firestore.collection('users-theme').doc(currentUser.uid).set(
          {
            theme: color,
          },
          { merge: true }
  ...
  loadUserTheme(userId: string) {
    this.firestore
      .collection('users-theme')
      .doc(userId)
      .get()
      .subscribe((doc) => {
        if (doc.exists) {
          const data = doc.data() as UserTheme;
          const theme = data.theme;
          if (theme) {
            this.setGradient(theme);
          }
        }
      });
  }
